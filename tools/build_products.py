#!/usr/bin/env python3
"""Generate halaman produk dari tools/products.json -> produk/<slug>/index.html
Jalankan: python3 tools/build_products.py"""
import json
import os
...
WA_FALLBACK = "Halo%20Bikinbaju%2C%20saya%20mau%20tanya%20harga%20seragam.%20Bisa%20dibantu%3F"
...
HEADER = '''<header class="site-header">
    <div class="container header-inner">
      ...
      <div class="header-cta">
        <a class="btn btn-wa" href="{wa_full}" rel="noopener" target="_blank">{wa_svg}<span>Tanya Harga</span></a>
...
def build_page(p, all_products):
    slug = p["slug"]
    img = p.get("image", f"/assets/img/produk/{slug}.svg")
    img_url = "https://bikin-baju.com" + img
    url = f"https://bikin-baju.com/produk/{slug}/"
    wa_full = p.get("wa_base", WA_FALLBACK) if "wa_base" in p else "https://wa.me/6281211671157?text=" + p["wa_text"]

    fabrics = "\n".join(
        f'          <li><strong>{b[0]}</strong> — {b[1]}</li>' for b in p["fabrics"]
    )
    specs = "\n".join(
        f'              <tr><th scope="row">{s[0]}</th><td>{s[1]}</td></tr>' for s in p["specs"]
    )
    uses = "\n".join(f'          <li>{u}</li>' for u in p["uses"])
    faqs = "\n".join(
    ...
    related = [o for o in all_products if o["slug"] != slug][:3]
    related_html = "\n".join(
        f'''          <article class="product-card">
            <div class="thumb"><img src="{o.get('image', '/assets/img/produk/' + o['slug'] + '.svg')}" alt="{o['name']} produksi Bikinbaju" loading="lazy" width="480" height="360"></div>
            <div class="body">
              <h3><a href="/produk/{o['slug']}/">{o['name']}</a></h3>
              <span class="price-chip">{o['price_label']}</span>
              <a class="card-link" href="/produk/{o['slug']}/">Lihat detail →</a>
            </div>
          </article>''' for o in related
    )

    product_schema = json.dumps({
        "@context": "https://schema.org",
        "@type": "Product",
        "name": f"{p['name']} Seragam Custom",
        "description": p["meta_desc"],
        "image": img_url,
        "brand": {"@type": "Brand", "name": "Bikinbaju"},
        "manufacturer": {"@id": "https://bikin-baju.com/#bisnis"},
        "offers": {
            "@type": "AggregateOffer",
            "url": url,
            "priceCurrency": "IDR",
            "lowPrice": str(p["price"]),
            "offerCount": "6",
            "availability": "https://schema.org/InStock",
            "seller": {"@id": "https://bikin-baju.com/#bisnis"},
        },
    ...
    breadcrumb_schema = json.dumps({
        ...
        "@type": "BreadcrumbList",
        "itemListElement": [
            {"@type": "ListItem", "position": 1, "name": "Beranda", "item": "https://bikin-baju.com/"},
            {"@type": "ListItem", "position": 2, "name": "Produk", "item": "https://bikin-baju.com/#produk"},
            {"@type": "ListItem", "position": 3, "name": p["name"], "item": url},
        ],
    }, ensure_ascii=False, indent=2)

    header = HEADER.format(wa_full=wa_full, wa_svg=WA_SVG)
    ...
    <section class="page-hero">
      <div class="container">
        <nav class="breadcrumb" aria-label="Breadcrumb">
          <a href="/">Beranda</a><span aria-hidden="true">/</span><a href="/#produk">Produk</a><span aria-hidden="true">/</span>{p['name']}
        </nav>
        <h1>{p['h1']}</h1>
        <p class="page-sub">{p['tagline']}</p>
        <span class="price-chip">{p['price_label']} / pcs</span>
      </div>
    </section>
    ...
    <section>
      <div class="container product-layout">
        <div class="product-photo">
          <img src="{img}" alt="{p['name']} seragam produksi konveksi Bikinbaju" width="640" height="800">
        </div>
        <div class="product-content">
          <h2>Deskripsi Produk</h2>
          <p>{p['intro']}</p>
          <p style="margin-top:12px">{p['intro2']}</p>
...
{uses}
          ...
          <div class="product-cta">
            <p>Minta penawaran {p['name'].lower()}?<small>Balasan cepat di jam kerja — Senin–Sabtu 08.00–17.00 WIB.</small></p>
            <a class="btn btn-wa" href="{wa_full}" rel="noopener" target="_blank">{WA_SVG} Tanya Harga</a>
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="stitch-top" style="background:var(--teal-50);padding-top:clamp(48px,6vw,72px)">
      <div class="container">
        <div class="section-head center">
          ...
          <h2 class="section-title">Pertanyaan Seputar {p['name']}</h2>
        </div>
        <div class="faq-list">
{faqs}
        </div>
    ...
    <section class="cta-band">
      <div class="container">
        ...
        <div class="hero-actions">
          <a class="btn btn-wa" href="https://wa.me/6281211671157?text=Halo%20Bikinbaju%2C%20saya%20mau%20tanya%20harga%20seragam.%20Bisa%20dibantu%3F" rel="noopener" target="_blank">Konsultasi Gratis</a>
          <a class="btn btn-outline" href="/#produk">Lihat Semua Produk</a>
        </div>
      </div>
    </section>
...
def main():
    ...
    for p in products:
        out_dir = os.path.join(BASE, "produk", p["slug"])
        os.makedirs(out_dir, exist_ok=True)
        out_file = os.path.join(out_dir, "index.html")
        with open(out_file, "w", encoding="utf-8") as f:
            f.write(build_page(p, products))
        print("OK", os.path.relpath(out_file, BASE))
